...
更新时间：2026-09-01
当前版本：`0.9.4`（`versionCode 15`）；本文只定义聊天与悬浮窗 UI 基线，版本交付证据以 `demo-app-version-ledger.md` 为准。
`0.9.3` 在保持既有聊天、过程与悬浮窗视觉契约的前提下，更新输入区、附件与多图交互基线（见下节）；`0.9.4` 为稳定性修复 patch，不改变该交互基线与视觉契约。设备与 skill 行为边界以版本台账当前条目为准。
