...
## D-024：Runtime capability assembly 统一静态与动态 skills

- 日期：2026-08-29
- 状态：已授权实施，阶段 6；基线为干净 `47964b5952a52f9d1194805c3a798abef5db035c`
- 背景：架构审查的 capability-assembly 段发现，`AgentRuntime.Builder.skillProvider()` 会清空已注册的 plugin-declared skills，`DemoAgentRuntimeFactory` 因而必须把同一份 `capabilityPlugins`、`SkillRepository` 和 `embedRoots` 重复传给 `FileBackedSkillProvider` 与 `AgentSkillRuntimePlugin`，并依赖注册/设置顺序。文件型 skill runtime 的动态 provider 也不应承担收集其它 plugin skills 的 assembly 职责。
- 决策：扩展 `AgentCapabilityPlugin` 一个默认空的 `skillProviders(): List<AndroidSkillProvider>` seam，plugin 可一次贡献 0..n 个动态 provider；既有 plugin 无需实现即可继续工作。`AgentRuntime.Builder.register(plugin)` 拒绝 blank 或重复 plugin id，避免 provider/lifecycle 重复挂载；Builder 是唯一的 skill assembly owner：注册时保留 plugin 引用、instructions 和 lifecycle plugin；`skillProvider(custom)` 只保留一个可替换的 custom provider，不再清空 plugin-declared skills。Runtime 每次 run 通过组合 provider 依次查询各 plugin 的 dynamic `skillProviders()` 及其 provider、custom provider（若设置）和各 plugin 的 `skills()`（plugin 注册顺序）。顺序按注册顺序稳定保持；plugin/provider/custom/plugin-declared/file-backed 任意来源的 skill id 必须非空且精确区分大小写，任何重复 id 都以包含冲突来源的描述失败，不进入 resolver 或模型；空 provider、空列表和无 custom provider 都是合法输入。
- 来源契约：`AndroidSkillProvider` 默认声明 `AndroidSkillProviderSource.GENERIC`；只有明确读取文件型 load-policy repository 的 provider 才声明 `FILE_BACKED`。Runtime 每轮把该 provider 实际返回的 file-backed ids 放入 `AndroidSkillResolutionContext.fileBackedSkillIds`，通过 resolver 的 source-aware overload 传递。`LoadPolicySkillResolver` 只对这些显式来源应用 repository 的 always/indexed 策略，绝不按 id、description 或 instructions 推断来源；因此 repository 中同名的 always manifest 不会改变 custom/plugin-declared skill 的语义。
- fail-closed 验收：来源契约的 repository 同 ID custom 回归、同一/跨 provider 与跨 plugin-declared duplicate、blank/Foo-vs-foo、Java null list/null element、`skillProviders()`/`skills()` 异常均在 assembly 阶段失败；resolver、LLM 不调用，session 只保留本次 user。
- 公共直构造边界：保留 `AgentRuntime` public constructor；传入单个普通 `AndroidSkillProvider` 时使用与 Builder Composite 完全相同的 file-private accumulator，执行相同的 null/blank/exact duplicate/source 校验并生成相同的 resolution context；直构造不承担多来源合并。assembly transport 与 file-backed provenance 只存在于 `AgentRuntime.kt` 的 file-private 实现；不提供可由 consumer 实现的 public/internal assembly marker，Builder Composite 也不向 public seam 暴露绕过 accumulator 的入口。
- 文件型 runtime：`AgentSkillRuntimePlugin` 作为完整单入口，在构造时接收 `repository`、`memoryRoot`、`embedRoots` 和既有 confirmation policy；它贡献 tools、instructions 与一个显式声明 `FILE_BACKED` 的动态 `FileBackedSkillProvider`。`FileBackedSkillProvider` 只接收 `repository` 和 `embedRoots`，每次 `skills()` 仍实时扫描 repository 并读取 embed；不再接收 plugin 列表或负责收集 plugin skills。Demo 只构造并注册 capability plugins，resolver 仍可单独持有 `SkillRepository` 以执行选择策略。
- 有意 semantic/API change：`AgentCapabilityPlugin` 新增默认 `skillProviders()`；`AndroidSkillProvider` 新增默认来源声明 `source`；`AndroidSkillResolver` 新增带 `AndroidSkillResolutionContext` 的默认 overload；`AgentRuntime.Builder.skillProvider()` 从“替换 plugin skills”改为“与 plugin dynamic/plugin-declared skills 合并”；`FileBackedSkillProvider` 的 public constructor 从 `(plugins, repository, embedRoots)` 改为 `(repository, embedRoots)`。默认方法的当前 JVM `javap` 证据表明既有实现可省略新增方法，但未对完整旧 consumer AAR ABI 做验收；Builder 无 custom provider 的既有 plugin-declared 路径保持可用，custom provider 的替换语义按本决策有意改为 merge；后一 constructor 及依赖旧替换语义的调用方需要迁移。不提供隐式兼容重载或静默旧顺序。当前仓库处于 `0.x`，本轮已明确授权该公共 seam 与语义调整。
- jvmDefault 边界：D-024 不改变现有 Kotlin compiler/jvmDefault mode，也没有证据表明当前构建配置发生确定漂移；完整 ABI 验收与显式固定 compiler mode 的建议留到阶段 8。
- lifecycle/行为边界：不改变 plugin tools 注册、instructions 的 trim/distinct、`cancelAllPlugins()`、逆注册 `close()` 且每个 Runtime 只 attach/close 一次；不改变 Provider profile、transcript、Terminal/Screen 互斥、UI、依赖、权限、版本/tag/push/PR、设备、network 或 API 配置。dynamic provider 与 plugin `skills()` 不能在 build 时拍平，repository/embed 更新从下一次 run 起生效；provider 异常或 JVM null-contract 违规在 run 中转为 assembly Failed；`LoadPolicySkillResolver(repository)` 继续属于选择策略而非 assembly。
- 测试与回退：以 TDD 先验证 RED，再逐个 GREEN，覆盖 register/skillProvider 两种顺序、custom setter 替换、plugin/provider 每 run 动态查询、排序/重复 id/blank/Foo-vs-foo、assembly Failed 与 resolver/model/session 边界、重复 plugin id、AgentSkillRuntimePlugin 单独注册与 live repository/embed、无双注入及 lifecycle 一次 attach/close；运行 `:ugk-pi-android:testDebugUnitTest`、`:pi-agent-skill-runtime-android:testDebugUnitTest`、`:demo-app:testDebugUnitTest`、`:demo-app:compileDebugKotlin`、`git diff --check` 和目标 `rg` 检查，不做 assemble/device/network/API。若需回退，只撤销 D-024 对应的 capability assembly、FileBacked/AgentSkillRuntimePlugin、必要 Demo 接线、测试和本决策/事实源文档，恢复到上述干净基线；不触碰 D-023 或其它能力域。
- 版本与发布边界：不 bump 版本、不改变 Maven 坐标、不创建发布物，不移动 tag，不 push，不创建 PR；该决策和实现不构成 release。

## D-025：Terminal 与 Screen capability 的宿主互斥及通用 Tool interlock

- 日期：2026-08-29
- 状态：已授权实施，阶段 7；基线为干净 `9956116a96d8cebc7fd8aba778989397ee8a3a7e`
- 背景：阶段 6 之后的 Demo 接线仍把 screen 专用 guard、错误码和阻断回退文案放在 `pi-terminal-skill-android`；前台和后台分别维护 screen 工具判断，后台还用 `startsWith("screen_")`，并在每个 screen `ToolFinished` 后释放终端阻断。这样既让 Terminal 依赖宿主的具体 capability 语义，也会在同一 Run 的后续 ToolCall 到来前错误解锁。
- 决策：Terminal/Screen 的 capability ownership 由 Host/Demo 持有。`ugk-pi-android` 提供可复用的 `AgentToolDecorator` 与通用 `AgentToolInterlock` seam；Terminal 只接受该通用 decorator，并将它应用于 `TerminalAgentPlugin.tools()` 返回的全部四个 Tool：`terminal_bash_execute`、`local_http_server_start`、`local_http_server_status`、`local_http_server_stop`。有 confirmation wrapper 的 Tool 先完成内部包装，再由 decorator 置于最外侧；只读 status Tool 则由 decorator 直接包装。Terminal 不识别任何 screen 工具名、screen guard、screen 错误码、屏幕恢复动作或 screen-specific callback。互斥拒绝统一返回通用 capability interlock contract：`CAPABILITY_INTERLOCKED`，并带阻断 capability 标识和消息；被拦截时不进入 confirmation wrapper、不执行底层 Tool。
- 决策（Demo ownership）：Demo 只保留一份完整、精确、大小写规则明确的 screen workflow matcher。matcher 对输入先 `trim`，再按 `Locale.ROOT` 转小写，随后只做集合精确相等判断；不得用前缀匹配。foreground `MainActivity` 与 background `DemoScheduledTaskPromptExecutor` 共用同一个 matcher/interlock lifecycle seam。当前 workflow 集合包含 `launch_android_app`、`launch_android_app_intent`、`screen_read_ui_tree`、`screen_find_ui_element`、`screen_perform_action`、`screen_capture_visual`、`screen_visual_gesture`、`screen_gesture`、`screen_press_key`、`screen_global_action`。
- 决策（Run lifecycle）：每个 Agent Run 创建/绑定一个独立 interlock 状态；Run 开始时清空上一 Run 的 ownership。该 Run 首次发出 workflow ToolStarted 后取得 screen capability ownership，直到统一终止边界保持终端 capability 被阻断；单个 workflow ToolFinished 不得释放。Completed、Failed、显式 cancel 以及 collector/coordinator `finally` 都必须幂等释放并清理该 Run 的 ownership，Activity 前后台切换不得改变状态。
- 决策（runtime contract）：从 `pi-terminal-skill-android/src/main/assets/ugk/AGENTS.md` 移除 Android app automation、screen workflow 和 screen-specific recovery instructions；该文件只描述 Terminal 自身的 Headless Bash、命令、服务、确认、环境和失败事实。Android app/screen 能力契约依赖 `pi-system-skill-android` 已有的 `AndroidAutomationAgentPlugin` instructions，不在 Terminal runtime contract 中重复。
- 原因：把能力互斥归属放在组合层，使同一 Terminal decorator 能复用于其它 capability owner；通用 guard 的外侧位置保证短路发生在确认和副作用之前；Run 级 ownership 保证模型在 screen workflow 中切换 Tool 或收到多个 ToolCall 时不会出现瞬时竞态。
- 影响：`pi-terminal-skill-android` 的 public constructor 增加可选通用 decorator seam，并删除 screen-specific callback/实现；Demo runtime factory 为 Terminal 注入由 Host 持有的通用 decorator，前后台执行器均负责同一 interlock 的事件转发和终止清理。Demo 日志/恢复提示按通用 `CAPABILITY_INTERLOCKED` contract 适配，不再依赖 Terminal 层 screen 错误码。需要更新对应 JVM tests、runtime asset 边界检查及 Demo compile/test；不改变 Provider、transcript、UI、依赖、版本/tag/push/PR、设备、network 或 API 配置。
- 验证与回退：按 TDD 逐个验证 guard 外侧短路、normal/full authorization 顺序、exact matcher、Run 生命周期、foreground/background 共用接线、runtime asset 边界和 Terminal 无 screen symbol 泄漏；运行 `:ugk-pi-android:testDebugUnitTest`、`:pi-terminal-skill-android:testDebugUnitTest`、`:pi-system-skill-android:testDebugUnitTest`、`:demo-app:testDebugUnitTest`、`:demo-app:compileDebugKotlin`、`git diff --check` 与目标 `rg` 静态门禁，不做 assemble/device/network/API。若需回退，只撤销 D-025 对应的通用 Tool decorator/interlock、Terminal 注入、Demo ownership/lifecycle、runtime asset、必要测试和本决策记录，不触碰 D-024 及其它能力域。

## D-026：文件型 skill 单文件 authoring 闭环

- 日期：2026-08-30
- 状态：已实施，最终门禁与独立 closeout review 通过
- 背景：D-022/D-024 已提供文件型 skill 的发现、解析和每轮动态注入，但 Agent 只能把草稿写入普通 workspace 文档，无法安全地创建、更新或删除 `<filesDir>/agent-skills/<name>/SKILL.md`，也缺少完成校验定义。
- 决策：在 `pi-agent-skill-runtime-android` 内新增 `skill_save`、`skill_delete` 和完整 manifest 的 `skill_read`。`skill_save` 只接收结构化字段，固定写入 repository 直接子目录，默认拒绝覆盖；写入使用同目录临时文件与替换，并在写前/写后复用 `SkillManifestParser`/`SkillRepository` 校验。`skill_delete` 只接收合法 name，canonical 校验后删除单个 repository 子目录。`agent-memory` 与内置 `android-skill-creator` 永远受保护。
- 决策（Agent SOP）：assets 内置 `android-skill-creator`，采用 `indexed`，只说明 UGK Android 单文件格式以及 create/update/list/read/delete/use SOP；普通 `docs/*.md` 不视为已安装 skill。Agent 只有在 `skill_save` 成功、`skill_list` 显示 `valid` 且 `skill_read` 核对 manifest/body 后才能报告创建或更新完成。
- 安全与确认：`skill_save`、`skill_delete` 与既有 `memory_delete` 默认通过 `UserConfirmationRequiredTool` 包装；full authorization 继续沿用 `shouldBypassConfirmation`，不改变工具自身的路径、格式或 protected-skill 校验。不存在显式 register 操作，保存后由现有 provider 在下一次 run 扫描生效。
